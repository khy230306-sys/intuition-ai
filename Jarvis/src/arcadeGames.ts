...
export const ARCADE_META: Record<ArcadeId, { title: string; blurb: string }> = {
  ...
  gyeokpa: { title: '격파', blurb: '세로 슈팅 · 레이저 7초 · 아군 최대 3기(10초)' },
}
...
/** —— 격파 (vertical shmup) —— */
export const GYEOKPA_LASER_SEC = 7
export const GYEOKPA_ALLY_SEC = 10
...
export function mountGyeokpa(canvas: HTMLCanvasElement, onScore?: ScoreCb): ArcadeHandle {
  ...
  type Ally = { slot: number; x: number; y: number; life: number; fireCd: number }
  ...
  function hudTitle(): string {
    ...
    const a = allies.length ? ` +${allies.length}` : ''
    return `격파 ${baseWeapon}${a}`
  }
  ...
  function pickupLaser(): void {
    ...
    flashLabel = laserCount > 1 ? `레이저 x${laserCount}` : '레이저'
  ...
  function pickupAlly(): void {
    if (allies.length >= GYEOKPA_MAX_ALLIES) {
      ...
      for (const a of allies) if (a.life < weakest.life) weakest = a
      weakest.life = GYEOKPA_ALLY_SEC
      flashLabel = '아군 갱신'
      flashUntil = performance.now() + 900
      return
    }
  ...
  function killEnemy(e: Enemy): void {
    ...
    if (noted.levelUpUntil) levelUpUntil = noted.levelUpUntil
    if (Math.random() < 0.2) {
      const kinds: Power['kind'][] = ['weapon', 'laser', 'ally', 'shield', 'laser', 'ally']
      powers.push({
        x: e.x,
        y: e.y,
  ...
  function draw(): void {
    ...
    for (const a of allies) {
      ctx.globalAlpha = 0.55 + Math.min(0.4, a.life / GYEOKPA_ALLY_SEC)
