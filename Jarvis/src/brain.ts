...
function helpText(name: string): string {
  return [
    ...
    '• 앱 업데이트 / 게임 / 벽돌깨기 / 스페이스 / 스페이스2 / 플래피 / 닷지 / 퐁 / 스윽 / 게임 순위',
    '• 내 위치 / 지금 어디야',
...
export async function think(
  ...
  if (/스페이스\s*2|스페이스투|격파|세로\s*슈팅|gyeokpa/i.test(text) && text.length < 28) {
    return {
      text: '스페이스2를 엽니다. 25스테이지 웨이브·보스가 나오고, 무기 아이템으로 펄스→트윈→스프레드→레이저로 강화됩니다. 라이프·실드·폭탄도 있습니다.',
      speak: true,
      view: 'games',
      arcadeId: 'gyeokpa',
    }
  ...
  if (/슈팅/i.test(text) && text.length < 20) {
    return {
      text: '스페이스2를 엽니다. 25스테이지 웨이브·보스가 나오고, 무기 아이템으로 펄스→트윈→스프레드→레이저로 강화됩니다.',
      speak: true,
      view: 'games',
      arcadeId: 'gyeokpa',
    }
  ...
  if (/과일\s*받|캐치|catch|두더지|mole|차\s*피하|레인|lanes|차선/i.test(text) && text.length < 28) {
    return {
      text: '그 게임은 삭제되었습니다. 스페이스 · 스페이스2 · 플래피 · 닷지 · 퐁 · 벽돌깨기 · 스윽을 이용해 주세요.',
      speak: true,
      view: 'games',
      arcadeId: 'shooter',
    }
