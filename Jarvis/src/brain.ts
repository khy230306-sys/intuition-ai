...
function helpText(name: string): string {
  return [
    ...
    '• 앱 업데이트 / 게임 / 벽돌깨기 / 스페이스 / 격파 / 플래피 / 닷지 / 퐁 / 스윽 / 게임 순위',
    '• 내 위치 / 지금 어디야',
...
export async function think(
  ...
  if (/격파|슈팅|세로\s*슈팅|gyeokpa/i.test(text) && text.length < 28) {
    return {
      text: '격파 세로 슈팅을 엽니다. 처음엔 메인기만 있고, 아군 아이템으로 최대 3기까지 늘립니다. 적 미사일에 맞으면 하나씩 줄고, 레이저는 10초입니다.',
      speak: true,
      view: 'games',
      arcadeId: 'gyeokpa',
    }
  ...
  if (/과일\s*받|캐치|catch|두더지|mole|차\s*피하|레인|lanes|차선/i.test(text) && text.length < 28) {
    return {
      text: '그 게임은 삭제되었습니다. 스페이스 · 격파 · 플래피 · 닷지 · 퐁 · 벽돌깨기 · 스윽을 이용해 주세요.',
      speak: true,
      view: 'games',
      arcadeId: 'shooter',
    }
