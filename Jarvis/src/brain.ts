...
function helpText(name: string): string {
  return [
    ...
    '• 앱 업데이트 / 게임 / 벽돌깨기 / 스페이스 / 스페이스2 / 플래피 / 지오대시 / 스윽 / 게임 순위',
    '• 내 위치 / 지금 어디야',
...
export async function think(
  ...
  if (/스페이스\s*2|스페이스2|격파|세로\s*슈팅|gyeokpa|space\s*2/i.test(text) && text.length < 28) {
    return {
      text: '스페이스2 세로 슈팅을 엽니다. 스테이지 25까지 진행하고, 웨이브·보스가 나옵니다. 무기는 펄스→트윈→스프레드→쿼드, 라이프·실드·폭탄도 있습니다.',
      speak: true,
      view: 'games',
      arcadeId: 'gyeokpa',
    }
  ...
  if (/닷지|dodge|장애물\s*피하|^퐁$|핑퐁|pong/i.test(text) && text.length < 24) {
    return {
      text: '닷지·퐁은 삭제되었습니다. 스페이스 · 스페이스2 · 플래피 · 지오대시 · 벽돌깨기 · 스윽을 이용해 주세요.',
      speak: true,
      view: 'games',
      arcadeId: 'shooter',
    }
  ...
  if (/과일\s*받|캐치|catch|두더지|mole|차\s*피하|레인|lanes|차선/i.test(text) && text.length < 28) {
    return {
      text: '그 게임은 삭제되었습니다. 스페이스 · 스페이스2 · 플래피 · 지오대시 · 벽돌깨기 · 스윽을 이용해 주세요.',
      speak: true,
      view: 'games',
      arcadeId: 'shooter',
    }
