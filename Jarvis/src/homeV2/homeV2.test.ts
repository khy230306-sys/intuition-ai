...
describe('HOME v2 render', () => {
  ...
  it('top 홈+메뉴; sheet points to 5 primary destinations', () => {
    ...
    expect(more).toContain('주요 메뉴')
    expect(more).toContain('바로가기')
    expect(more).toContain('data-view="home"')
    expect(more).toContain('data-view="chat"')
    expect(more).toContain('data-view="schedule"')
    expect(more).toContain('data-view="family-helper"')
    ...
    expect(more).not.toContain('data-view="friends"')
    ...
    expect((more.match(/data-view="family-helper"/g) || []).length).toBe(1)
