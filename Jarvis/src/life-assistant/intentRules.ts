...
export function classifyLifeAssistantRules(text: string): LifeAssistantIntentResult | null {
  ...
  if (
    /(이\s*)?(문서|사진|안내문|알림장).*(읽어|번역|일정|분석)|카메라\s*(열어|켜)|사진\s*(찍어|분석)|음식\s*뭔지|준비물\s*안내문/.test(
      t,
    )
  ) {
  ...
  // Parking
  if (/주차\s*(위치|한\s*곳|한곳).*(기억|저장)|주차\s*기억|주차했어|여기\s*주차/.test(t)) {
    const loc = t.match(/(?:위치|곳|장소)(?:은|는)?\s*(.+)$/)?.[1]?.trim()
    return base(t, {
      intent: 'parking.save',
      confidence: 0.94,
      location: loc,
      extractedEntities: { location: loc, note: loc },
    })
  }
  if (/주차\s*(위치|한\s*곳|한곳).*(알려|어디|보여)|내가\s*주차|주차\s*어디/.test(t)) {
  ...
  if (
    (/(\d+\s*분|\d+\s*시간)\s*(뒤|후)/.test(t) && /알려|알림|리마인더/.test(t)) ||
    (/약\s*먹/.test(t) && /알려|알림/.test(t)) ||
    (/알림\s*(설정|등록|추가)/.test(t) && /(\d+\s*분|\d+\s*시간|시)/.test(t))
  ) {
    const title =
      extractTitle(t, /\d+\s*분\s*(뒤|후)|\d+\s*시간\s*(뒤|후)|알려줘|알림|해줘|리마인더/g) || '알림'
    return base(t, {
  ...
  // Family schedule
  if (/가족\s*일정|아이\s*(예방접종|하원|등교)|엄마\s*.*병원|한영이?\s*.*(하원|등교|학원)/.test(t)) {
    if (/추가|등록|넣어|잡아/.test(t)) {
      const title = extractTitle(t, /가족\s*일정|추가|등록|해줘/g) || '가족 일정'
      const person = extractPerson(t)
      const missing: string[] = []
      if (!extractKoreanDate(t) && !/오늘|내일|모레|다음/.test(t)) missing.push('date')
      return base(t, {
    ...
    return base(t, { intent: 'family.schedule.read', confidence: 0.92, person: extractPerson(t) })
  }
  ...
  // Calendar create
  if (
    /(일정|예약)\s*(추가|등록|잡아|넣어)|병원\s*예약\s*추가|캘린더에\s*넣/.test(t) ||
    (/(병원|회의|약속)/.test(t) && /(추가|등록|잡아)/.test(t))
  ) {
    const title =
      extractTitle(t, /일정|예약|추가|등록|잡아|넣어|해줘|캘린더에/g) ||
      (/병원/.test(t) ? '병원' : undefined)
    const missing: string[] = []
    if (!title) missing.push('title')
    if (!extractKoreanDate(t) && !extractKoreanTime(t)) missing.push('date')
    return base(t, {
