...
describe('local alarms', () => {
  ...
  it('parses relative and absolute times', () => {
    ...
    expect(wantsLocalAlarm('오후 3시에 알려줘 회의')).toBe(true)
    expect(wantsLocalAlarm('내일 내가있는곳의 날씨를알려줘')).toBe(false)
    expect(wantsLocalAlarm('내일하루종일 날씨좀 알려줘 내가있는 위치')).toBe(false)
    expect(wantsLocalAlarm('날씨 알려줘')).toBe(false)
    expect(wantsLocalAlarm('환율 알려줘')).toBe(false)
    expect(wantsLocalAlarm('오늘 뉴스')).toBe(false)
...
describe('brain wiring for practical features', () => {
  ...
  it('handles market hours, expense, and alarm intents', async () => {
    ...
    expect(a.text).toMatch(/알람을 맞춰 두었어요|알림 예약|알려 드릴게요|알려드릴게요/)
  })
