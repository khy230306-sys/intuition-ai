...
export function isTranslateEscapeCommand(text: string): boolean {
  ...
  // Active translation mode owns the turn — including weather *queries*.
  // 「울산 날씨 알려줘」 must be translated, not routed to the weather tool.
  // Stop / language-switch are handled separately; only hard navigation escapes.
  return /(?:으로\s*)?안내해\s*줘|길\s*안내|내비(?:게이션)?|네비|전화해/i.test(t)
}
