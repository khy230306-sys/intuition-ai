  ...
  return (
    <div>
      ...
      <div className="grid-2">
        {missions.map((m) => {
          ...
          return (
            <Link
              ...
              to={`/games/${m.id}`}
              className="card"
              style={{ opacity: m.done ? 0.7 : 1, background: m.done ? '#e8f8ee' : undefined }}
            >
        ...
        <Link to="/games/sticker-book" className="card" style={{ background: '#fff3c4' }}>
      ...
      <p className="section-sub">사운드·동화·퍼레이드·퍼즐로 더 오래 놀아요</p>
      <div className="grid-2">
        {featured.map((g, i) => (
          <Link key={g.id} to={`/games/${g.id}`} className="card" style={{ animationDelay: `${i * 0.05}s` }}>
            <div className="card-emoji">{g.emoji}</div>
            <div className="card-title">{g.title}</div>
